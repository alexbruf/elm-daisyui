...
| **Leaf** | A terminal control or piece of content. Contains only data (`String`, `Float`, config), never another node. Leaves live inside Blocks, inside `Navbar`, or inside `Toast`/`Modal` via a Block. The one leaf that holds something other than data is `Leaf.Embed`, which holds a `ThemeContext -> Html msg` — still terminal, because what comes back is drawn inside a fixed box and no tree node can be reached from it. |
...
(inside `Block.Prose`) and for assistive technology to read. `Leaf.Embed` is the fifth and the only one that is not a picture of anything daisyUI ships: it carries a
`ThemeContext -> Html msg` for a drawing the tree has no component for, boxed and labelled by the renderer (see
its row below). `Leaf.Icon` is the fourth, for the same reason
...
| *(embed)* | Leaf | `Leaf.Embed EmbedConfig (ThemeContext -> Html msg)` | none — the function's `Html` is drawn inside the renderer's box | **Not a daisyUI component.** The one door for a custom drawing daisyUI has no component for and `Block.Chart`'s five kinds cannot express (the demo's conversion funnel, `demo/src/Viz/Funnel.elm`). It is a `Leaf`, so it cannot hold or replace a block, a section or an overlay; `Daisy.Render` wraps it in a fixed box (`relative overflow-hidden w-full` + one of `EmbedSm`/`EmbedMd`/`EmbedLg` = 160/256/384px, all `Render.tokens`) with `role="figure"` and `EmbedConfig.label` as `aria-label`, so it cannot overlap a neighbour, escape its box or go unnamed. It emits no class of its own and may not: `NoClassOutsideRender` forbids `Html.Attributes.class` inside an embed and `NoRawSchemaStrings` forbids a daisyUI class as a literal, so it paints with inline `style`/SVG attributes and the `var(--color-*)` values in `ThemeContext`. |
...
| fieldset | Parts-of | part record inside `Block.Form`: `Fieldset msg = { legend : Maybe String, columns : FieldsetColumns, fields : List (Field msg) }` | `List (Field msg)` | `fieldset-legend` is a `part`, and a fieldset only ever means something inside a form. Making it a record makes a stray legend unrepresentable. `columns` (`OneColumn \| Columns2`) lays the run of fields in one or two tracks: it is on the fieldset because the run of fields is what is being laid out — two fieldsets in one form can want different shapes — and `Daisy.Render` wraps the fields in the grid rather than putting it on the `<fieldset>`, whose `display: grid` would give the **legend** the first cell (2026-09-07). |
...
| (icon) | Leaf | `Leaf.Icon` | `IconConfig` (`size : IconSize`, `tone : Maybe IconTone`, `label : Maybe String`) + `Daisy.Icon.Icon` | Not a daisyUI component and emits no daisyUI class, like `Leaf.Heading` and `Leaf.Image`. A closed set of 38 heroicons outline drawings; the path data lives in the internal `Daisy.Render.Icons`, so `Daisy.Icon` stays pure data and no caller can hand the renderer markup. Also reachable as `MenuGlyph.MenuIcon` (the leading glyph of a `MenuItem`, which since 2026-09-07 is a closed `MenuIcon` / `MenuThemeDots` pair rather than a bare `Maybe Icon`), `ButtonConfig.icon` and `Cta.icon`, which is where daisyUI's dashboard templates put glyphs. `tone` (closed at six, added 2026-09-08) is the one semantic *foreground* in the package: an icon holds no text node, so a tone can never colour a word, which is why `text-primary` and its four siblings became `Daisy.Render` tokens here and nowhere else. |
...
| (color chips) | Leaf | `Leaf.ColorChips` | `List (ColorChipGroup msg)`, each `{ label, chips : List ColorChip }` | Not a daisyUI component and emits no daisyUI class. `Leaf.Swatch` shows a colour a theme *has*; this edits one a theme does **not** have yet, which is the difference that makes it a second leaf rather than a flag on the first: the colour is an `Oklch` value painted with an inline `background-color`, because there is no `--color-*` for it to be a utility of. One chip is a painted square with a native `type="color"` picker lying over it (`relative` wrapper, `absolute inset-0 opacity-0`), named by the variable it edits. A whole editor rather than one chip per leaf, because the layout is a grid of *groups* — daisyUI's `grid-cols-4` with a `col-span` per chip count — and a `card-body` is a column that cannot arrange leaves in rows. `ChipGlyph` is the closed three-value glyph (`ChipBlank`, `ChipLabel`, `ChipSpecimen`). Added 2026-09-07; see `docs/tree-decisions.md`, "The generator's editor column". |
| (radius tiles) | Leaf | `Leaf.RadiusTiles` | `RadiusTilesConfig` + `{ group : String, current : Radius }` | Not a daisyUI component, but it emits daisyUI's: a `join` of `btn btn-sm` `<label>`s over `sr-only` radios. It is one leaf and not five, for the same reason `Leaf.Filter` is one leaf: a radio group is one control, and five loose radios could not be made exclusive by the type. Each step *draws* the corner it sets, from an inline `border-start-end-radius` — a radius is a value out of a five-member set, and no utility spells `--radius-box`'s steps. The five options are always `allRadii`. Added 2026-09-07. |
| (theme dots) | Leaf | `Leaf.ThemeDots`, and `MenuGlyph.MenuThemeDots` | `Theme` | Not a daisyUI component and emits no daisyUI class. Four dots of a theme's own `base-content` / `primary` / `secondary` / `accent` on a tile of its `base-100`, all four written inline: the tile is a picture of *another* theme, so `bg-primary` on it would paint the page's theme instead of the row's. `Daisy.Render` reads the values out of the generated `Daisy.Themes` (a `Theme.Custom` already carries them). It is also a `MenuGlyph`, because the place it belongs is the leading glyph of a theme-list row and a `Leaf` cannot sit inside a `MenuItem`. Added 2026-09-07. |
...
| list | Block | `Block.ListBlock` + `ListConfig` | `List (ListRow msg)`, each `{ cells : List (ListCell msg) }`, a cell being `{ content : Leaf msg, grow : Bool, wrap : Bool }` | A `<ul class="list">` of `list-row` items holding images, text and buttons: structurally a Table-like Block. `list-col-grow` / `list-col-wrap` mark one *cell*, so they are flags there. `ListConfig` (added 2026-09-08) has one field, `style : ListPanel \| ListRules`: `ListPanel` is that component, `ListRules` is the compact shape daisyUI's own theme generator uses instead — rows with `py-2` and a dashed hairline, and **no** `list` class at all, because `list-row`'s gutter is a fixed `1rem` that no size class changes. |
...
| table | Block | `Block.Table TableConfig (List (Row msg))` | `List (Row msg)` where `Row = { header : Bool, cells : List (TableCell msg) }` and `TableCell = { leading : Maybe (Leaf msg), content : Leaf msg, truncate : Bool }` | Fixed by the spec. Rows hold Leaves, which is what the Admin demo's badge + row-action button needs. `truncate` is on the **cell** because exactly one column of a dashboard table is the one that may be clipped; `Daisy.Render` emits `truncate max-w-0 w-full` together (none of the three clips anything alone) and gives every other cell of that row `whitespace-nowrap`, because a percentage-width column in an auto table layout starves its neighbours to min-content (2026-09-07). |
...
| "Start from" any built-in or `acme` | `Block.Menu` of theme rows, each a `MenuItem` whose `glyph` is `MenuThemeDots <that theme>`; clicking one is the `StartFrom` edit. `Daisy.Themes.builtinToCustom` supplies the values behind a built-in's name |
| One picker per `--color-*` variable | one `Leaf.ColorChips` for all twenty: a painted square per variable with a native `type="color"` picker over it, named by the variable it edits; `Daisy.Color` converts `#rrggbb` <-> `oklch()` |
| Radius steps | `Leaf.RadiusTiles` per `--radius-*` variable — a radio group whose five steps are drawn at the corner each one sets |
| Size / border steps | `Leaf.Join` of `btn-xs` buttons over `allSizes` / `allBorders`, offered under the CSS length each step stands for |
| `--depth` / `--noise` / dark scheme | `Leaf.Toggle` — a `Bool` in the tree, a `0`/`1` in the CSS |
| The palette readout | `Leaf.ThemeDots (Custom edited)` then `Leaf.Swatch` per `SwatchColor` in `CardParts.body`, badges in `CardParts.actions` (`card-actions` is `flex flex-wrap`; a `card-body` child would stretch to the full width) |
| Live preview | ordinary `Leaf.Button` / `Leaf.Badge` / `CardAlert` / `CardForm` / `CardChart` / `CardTable` — the whole page is the preview. Nineteen cards in one `GridItem` with `columns = CellThree`, which `Daisy.Render` deals into three `flex flex-col` columns so they pack per column as daisyUI's do |
...
**No longer a gap:** a chart shape `Block.Chart`'s five kinds cannot draw. `Leaf.Embed` covers it — the
Analytics demo's "Conversion funnel" card is a `CardLeaf (Embed ...)` over
`demo/src/Viz/Funnel.elm`, drawn with `gampleman/elm-visualization` and coloured only through
`ThemeContext`. This is not a general escape hatch: everything above stays a gap, because an embed is
for a drawing, not for hand-writing markup a `Block` or `Leaf` should express.
