...
- No `Raw Html` constructor. Custom markup has exactly one door, `Leaf.Embed`, and it is a framed one: `Embed EmbedConfig (ThemeContext -> Html msg)`. The function is called by `Daisy.Render` with a `ThemeContext` (`color : SemanticColor -> String`, `surface : Surface -> String`, `theme : Theme`, `radiusBox : String` — all `var(--*)` values, no classes) and its result is drawn inside a fixed box (`relative overflow-hidden w-full` plus one of three fixed height tokens, all `Render.tokens`) carrying `role="figure"` and the config's `label` as `aria-label`. It is a `Leaf`, so it cannot hold a block, a section or an overlay, and it cannot take one's place. `NoClassOutsideRender` and `NoRawSchemaStrings` still apply inside an embed, so it cannot reach a daisyUI class at all — it styles itself with inline `style`/SVG attributes and the `ThemeContext` colours. What an embed costs is visibility: `CoverageTest`, the corpus and `render-class-audit` read classes back off markup and an embed emits none. Overlap, contrast, a11y and the theme sweeps still see it, because they read the painted page. A docs example that still cannot be expressed goes in `rejected.md`, not in the type — an embed is for a *drawing* the tree has no component for, not for reproducing daisyUI markup by hand.
