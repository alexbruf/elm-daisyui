module Daisy.Render.Icons exposing (paths)

{-| The path data behind `Daisy.Icon`.

Internal to the package: not listed in `elm.json`'s `exposed-modules`, so no
application can reach it. It lives beside `Daisy.Render` rather than inside
`Daisy.Icon` for the same reason `Daisy.Chart` holds no elm-charts calls —
`Daisy.Icon` is pure data, and everything that becomes markup belongs to the
renderer.

Every entry is the `d` attribute of one `<path>` of the matching **heroicons
2.2.0 outline** 24x24 drawing (`node_modules/heroicons/24/outline/<name>.svg`,
where `<name>` is `Daisy.Icon.name`), copied verbatim. heroicons is MIT,
(c) Tailwind Labs, Inc.; the licence text is quoted in the `Daisy.Icon` module
comment. It is a build-time source only: this package has no npm dependency.

The shared attributes every heroicons outline drawing carries — `viewBox`,
`fill="none"`, `stroke="currentColor"`, `stroke-width="1.5"` and the two
`stroke-line*` values — are not repeated per icon; `Daisy.Render.iconHtml`
writes them once.
...
{-| The `<path d="...">` values of one icon, in drawing order.
-}
paths : Icon -> List String
paths icon =
    case icon of
        Home ->
